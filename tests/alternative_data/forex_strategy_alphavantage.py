...
class AlphaVantageDataProvider:
    ...
    def get_forex_data(self, from_currency, to_currency, interval="daily", outputsize="compact"):
        ...
        try:
            ...
            # Parse the data
            time_series_key = "Time Series FX (Daily)" if interval == "daily" else f"Time Series FX ({interval.upper()})"
...
class StochasticRSIStrategy:
    ...
    def generate_signals(self, df, rsi_period=14, stoch_period=14):
        ...
        # Calculate Stochastic RSI
        k_percent, d_percent = self.calculate_stochastic_rsi(close_prices, rsi_period, stoch_period)
    ...
    def create_chart(self, signals, title, save_path=None):
        ...
        if not buy_points.empty:
            ax1.scatter(buy_points.index, buy_points['close'], color='green', s=100, marker='^', label='Buy Signal')
        if not sell_points.empty:
            ax1.scatter(sell_points.index, sell_points['close'], color='red', s=100, marker='v', label='Sell Signal')
        ...
        # Stochastic RSI
        ax3.plot(signals.index, signals['stoch_k'], color='#2ca02c', linewidth=1, label='%K')
        ax3.plot(signals.index, signals['stoch_d'], color='#d62728', linewidth=1, label='%D')
...
def main():
    ...
    if signals is not None:
        ...
        print(recent_signals[['close', 'rsi', 'stoch_k', 'stoch_d', 'buy_signal', 'sell_signal']].round(2))
