...
def debug_historical_data():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            request = InstrumentsCandles(instrument="EUR_USD", params=test_case["params"])
            ...
            if 'candles' in response:
                ...
                if candles:
                    ...
                    print(f"   First close: {first_candle.get('mid', {}).get('c', 'N/A')}")
                    print(f"   Last close: {last_candle.get('mid', {}).get('c', 'N/A')}")
                    
                    # Check if candles are complete
                    complete_candles = [c for c in candles if c.get('complete', False)]
                    print(f"   Complete candles: {len(complete_candles)}/{len(candles)}")
