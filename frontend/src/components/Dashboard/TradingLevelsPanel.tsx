...
interface TradingLevelsPanelProps {
  symbol: string;
  timeframe: string;
  onLevelAdd?: (level: TradingLevel) => void;
  onLevelRemove?: (levelId: string) => void;
}

const TradingLevelsPanel: React.FC<TradingLevelsPanelProps> = ({
  symbol,
  timeframe,
  onLevelAdd,
  onLevelRemove
  ...
  const handleAddLevel = () => {
    const level = tradingLevelsService.addLevel({
      ...newLevel,
      symbol,
      timeframe
    });
  ...
  const handleRemoveLevel = (levelId: string) => {
    tradingLevelsService.removeLevel(levelId);
    if (onLevelRemove) {
  ...
  return (
    <div className="bg-white p-4 rounded-lg shadow-md border border-gray-200">
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-semibold text-gray-900">Trading Levels</h3>
        <button
      ...
      {levels.length > 0 && (
        <div className="mt-4 pt-3 border-t border-gray-200">
          <div className="text-xs text-gray-500">
            Trading Levels: {levels.length} total • 
            {levelTypes.map(type => {
