...
class CandlestickStrategyViewer:
    ...
    def create_candlestick_charts(self, save_path: str = "candlestick_analysis.png"):
        ...
        fig.suptitle(
            f"Candlestick Strategy Analysis - {self.symbol}",
            fontsize=16,
            fontweight="bold",
        )
    ...
    def create_mplfinance_chart(self, save_path: str = "mplfinance_chart.png"):
        ...
        df = pd.DataFrame(
            ...
            index=[b.timestamp for b in self.bars],
        )
        ...
        if len(self.signals) > 0:
            ema_fast_data = pd.Series(
                [s.indicators["ema_fast"] for s in self.signals],
                index=[s.timestamp for s in self.signals],
            )
            ema_slow_data = pd.Series(
                [s.indicators["ema_slow"] for s in self.signals],
                index=[s.timestamp for s in self.signals],
            )
        ...
        if buy_signals:
            ...
            apds.append(
                mpf.make_addplot(
                    buy_data, type="scatter", markersize=100, marker="^", color="green"
                )
            )
        ...
        if sell_signals:
            ...
            apds.append(
                mpf.make_addplot(
                    sell_data, type="scatter", markersize=100, marker="v", color="red"
                )
            )
    ...
    def _print_statistics(self):
        ...
        if self.signals:
            ...
            ema_ratios = [
                s.indicators["ema_fast"] / s.indicators["ema_slow"]
                for s in self.signals
            ]
