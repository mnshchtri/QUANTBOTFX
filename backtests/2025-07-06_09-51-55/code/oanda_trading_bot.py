...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("oanda_bot.log"), logging.StreamHandler()],
)
...
class OANDAConnector:
    ...
    def get_candles(
        self, instrument: str, granularity: str = "M5", count: int = 500
    ) -> pd.DataFrame:
...
class RiskManager:
    ...
    def calculate_position_size(
        self, signal: TradingSignal, account_balance: float, current_price: float
    ) -> int:
...
class OANDATradingBot:
    ...
    def initialize(self):
        ...
        try:
            ...
            logger.info(
                f"Connected to OANDA account: {account_info['account']['name']}"
            )
    ...
    def _execute_signal(self, signal: TradingSignal, current_positions: Dict):
        ...
        try:
            ...
            # Log signal details
            logger.info(
                f"Signal executed: {signal.signal} {instrument} "
...
def main():
    ...
    if not config.account_id or not config.api_key:
        logger.error(
            "Please set OANDA_ACCOUNT_ID and OANDA_API_KEY environment variables"
        )
        return
