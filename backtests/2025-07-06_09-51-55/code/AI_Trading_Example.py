...
class SimpleAITradingStrategy(QCAlgorithm):
    ...
    def execute_trade(self, signal, price, confidence):
        ...
        if signal == "BUY":
            ...
            self.Debug(
                f"BUY: Price={price}, Size={adjusted_size}, Confidence={confidence:.3f}"
            )
            ...
            self.Debug(
                f"SELL: Price={price}, Size={adjusted_size}, Confidence={confidence:.3f}"
            )
...
class FeatureExtractor:
    ...
    def extract_features(self, price, volume, ema_short, ema_long, rsi):
        ...
        if len(self.volume_history) > 0:
            volume_ratio = (
                volume / np.mean(self.volume_history[-20:])
