    ...
    def update_hourly_data(self, bar):
        ...
        if not self.processed_first_two_hours:
            if current_hour == 9:  # First hour of trading
                ...
                self.algorithm.Debug("First two hours completed. Starting breakout detection.")

        # Evaluate breakout conditions after the first two hours
        confirmed_break = None
        
        if self.processed_first_two_hours and not self.bullish_breakout_in_one_hour and not self.bearish_breakout_in_one_hour:
