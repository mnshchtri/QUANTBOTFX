...
class ForexFeatureEngine:
    ...
    def __init__(self):
        ...
        self.exotic_pairs = ["USDZAR", "USDSGD", "USDHKD", "USDDKK"]
    ...
    def extract_forex_features(self, data: pd.DataFrame) -> Dict[str, float]:
        ...
        features["returns_20d"] = (close_prices[-1] - close_prices[-20]) / close_prices[
            -20
        ]
        ...
        features.update(
            self._extract_forex_breakout_features(high_prices, low_prices, close_prices)
        )
    ...
    def _extract_forex_breakout_features(
        ...
        features["position_in_range"] = (close_prices[-1] - support_20) / (
            resistance_20 - support_20
        )

        # Pin bar detection
        features["pin_bar_up"] = self._detect_pin_bar_up(
            high_prices, low_prices, close_prices
        )
        features["pin_bar_down"] = self._detect_pin_bar_down(
            high_prices, low_prices, close_prices
        )

        return features

    def _extract_currency_strength_features(
        ...
        # Momentum indicators
        features["momentum_5d"] = (close_prices[-1] - close_prices[-5]) / close_prices[
            -5
        ]
    ...
    def _detect_pin_bar_up(
        ...
        if (
            body_size < (upper_shadow + lower_shadow) * 0.3
        ...
        if not features:
            return ForexSignal(
                ...
                pip_value=self._calculate_pip_value(
                    symbol, data["close"].iloc[-1] if len(data) > 0 else 1.0
                ),
            )
...
class ForexRiskManager:
    ...
    def calculate_forex_position_size(
        self, signal: ForexSignal, account_balance: float, stop_loss_pips: int = 50
    ) -> int:
