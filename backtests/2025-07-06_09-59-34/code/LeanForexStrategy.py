...
class LeanForexStrategy(QCAlgorithm):
    ...
    def GenerateSignal(self, pair: str, bar: TradeBar) -> Dict:
        ...
        if not (
            indicators["ema_fast"].IsReady
            and indicators["ema_slow"].IsReady
            and indicators["rsi"].IsReady
    ...
    def CalculateBreakoutFeatures(self, pair: str, bar: TradeBar) -> Dict[str, float]:
        ...
        features["position_in_range"] = (bar.Close - support_20) / (
            resistance_20 - support_20
        )

        return features

    def CalculateCurrencyStrengthFeatures(
        self, pair: str, bar: TradeBar
    ) -> Dict[str, float]:
    ...
    def OnOrderEvent(self, orderEvent):
        ...
        if orderEvent.Status == OrderStatus.Filled:
            self.Debug(
                f"Order filled: {orderEvent.Symbol} {orderEvent.FillQuantity} @ {orderEvent.FillPrice}"
            )
    ...
    def OnEndOfAlgorithm(self):
        ...
        for pair in self.forex_pairs:
            if pair in self.Portfolio:
                ...
                if position.Quantity != 0:
                    self.Debug(
                        f"{pair}: {position.Quantity} units, P&L: {position.UnrealizedProfit}"
                    )
