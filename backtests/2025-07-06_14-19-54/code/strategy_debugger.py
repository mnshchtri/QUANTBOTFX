...
class StrategyDebugger:
    ...
    def _log_step(self, bar: BarData, signal_data: SignalData):
        ...
        print(f"[{bar.timestamp}] {signal_data.signal} @ {bar.close:.5f} "
    ...
    def create_comprehensive_charts(self, save_path: str = "strategy_analysis.html"):
        ...
        fig.suptitle(f'Strategy Analysis - {self.symbol}', fontsize=16, fontweight='bold')
        
        # 1. Price and EMAs
        ax1 = axes[0]
        ax1.plot(df_bars['timestamp'], df_bars['close'], label='Price', linewidth=1, alpha=0.8)
        ax1.plot(df_signals['timestamp'], df_signals['ema_fast'], label='EMA Fast', linewidth=1)
        ax1.plot(df_signals['timestamp'], df_signals['ema_slow'], label='EMA Slow', linewidth=1)
        ...
        # 2. RSI
        ax2 = axes[1]
        ax2.plot(df_signals['timestamp'], df_signals['rsi'], label='RSI', color='purple')
        ...
        # 3. MACD
        ax3 = axes[2]
        ax3.plot(df_signals['timestamp'], df_signals['macd'], label='MACD', color='blue')
        ax3.plot(df_signals['timestamp'], df_signals['macd_signal'], label='Signal', color='orange')
        ax3.bar(df_signals['timestamp'], 
        ...
        ax4.plot(df_signals['timestamp'], df_signals['confidence'], 
                label='Signal Confidence', color='green', linewidth=2)
        ax4_twin.plot(df_signals['timestamp'], df_signals['atr'], 
                     label='ATR', color='red', alpha=0.7)
