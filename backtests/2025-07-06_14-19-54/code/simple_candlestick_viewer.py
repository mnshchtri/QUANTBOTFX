...
class SimpleCandlestickViewer:
    ...
    def create_candlestick_chart(self, save_path: str = "simple_candlestick.png"):
        ...
        if sell_signals:
            ...
            sell_indices = [
                timestamps.index(ts) for ts in sell_times if ts in timestamps
            ]
            sell_prices_filtered = [
                price
                for i, price in enumerate(sell_prices)
                if sell_times[i] in timestamps
            ]
        ...
        if signal_indices:
            ...
            ax2.plot(
                signal_indices, rsi_values, label="RSI", color="purple", linewidth=2
            )
        ...
        if signal_indices:
            ...
            ax3.plot(
                signal_indices, macd_values, label="MACD", color="blue", linewidth=2
            )
            ax3.plot(
                signal_indices,
                macd_signal_values,
