...
class FifteenMinuteChart:
    ...
    def create_15min_chart(self, save_path: str = "15min_trade_chart.png"):
        ...
        ax1.axhline(y=first_hour_high, color='#ffeb3b', linestyle='--', linewidth=1.5, label='1st Hour High')
        ax1.axhline(y=first_hour_low, color='#ffeb3b', linestyle=':', linewidth=1.5, label='1st Hour Low')
        ax1.axhline(y=second_hour_high, color='#00bcd4', linestyle='--', linewidth=1.5, label='2nd Hour High')
        ax1.axhline(y=second_hour_low, color='#00bcd4', linestyle=':', linewidth=1.5, label='2nd Hour Low')

        # Mark breakouts
        for event in breakout_events:
            idx = timestamps.index(event[0])
            ax1.scatter(idx, event[2], marker='*', color='yellow', s=250, zorder=6, label=event[1])
        ...
        if buy_signals:
            ...
            if buy_indices:
                ax1.scatter(buy_indices, buy_prices, 
                           marker='^', color=self.colors['buy_signal'], s=200, 
        ...
        if sell_signals:
            ...
            if sell_indices:
                ax1.scatter(sell_indices, sell_prices, 
                           marker='v', color=self.colors['sell_signal'], s=200, 
                           label='Sell Signal', zorder=5, edgecolors='white', linewidth=2)
        
        # Format main chart
        ax1.set_title(f'{self.symbol} - 15-Minute Trade Chart (24h)', 
    ...
    def _print_signal_summary(self):
        ...
        if buy_signals or sell_signals:
            print(f"\n🎯 RECENT SIGNALS:")
            recent_signals = sorted(buy_signals + sell_signals, key=lambda x: x.timestamp)[-5:]
            for signal in recent_signals:
                print(f"   {signal.timestamp.strftime('%H:%M')} - {signal.signal} @ {signal.price:.5f} (Confidence: {signal.confidence:.3f})")
