...
class DetailedTradeAnalyzer:
    ...
    def _process_bar(self, bar: BarData):
        ...
        signal, confidence, signal_type, trade_reason = self._generate_comprehensive_signal(
            bar, prices[current_idx], ema_fast[current_idx], ema_slow[current_idx], 
            rsi[current_idx], macd_line[current_idx], signal_line[current_idx],
            bb_upper[current_idx], bb_lower[current_idx], atr[current_idx],
            breakout_analysis, reversal_analysis, first_hour_levels
        )
        ...
        signal_data = TradeSignal(
            ...
            first_hour_high=first_hour_levels.first_hour_high if first_hour_levels else 0,
    ...
    def _update_first_hour_levels(self, bar: BarData):
        ...
        if not self.first_hour_levels or self.first_hour_levels[-1].date.date() != current_date:
    ...
    def _analyze_breakouts(self, bar: BarData, first_hour_levels: Optional[FirstHourLevels]) -> Dict:
        ...
        if breakout_high:
            strength = (bar.close - first_hour_levels.first_hour_high) / first_hour_levels.first_hour_high
            return {'confirmed': True, 'type': 'HIGH', 'strength': strength}
        elif breakout_low:
            strength = (first_hour_levels.first_hour_low - bar.close) / first_hour_levels.first_hour_low
            return {'confirmed': True, 'type': 'LOW', 'strength': strength}
        else:
            return {'confirmed': False, 'type': None, 'strength': 0}
    
    def _analyze_15min_reversal(self, bar: BarData, prices: np.ndarray, current_idx: int) -> Dict:
        ...
        recent_volumes = [b.volume for b in self.bars[current_idx-14:current_idx+1]]
    ...
    def _calculate_support_resistance(self, prices: np.ndarray, current_idx: int) -> Dict[str, float]:
    ...
    def _calculate_risk_reward_ratio(self, bar: BarData, signal: str, atr: float) -> float:
    ...
    def _generate_comprehensive_signal(self, bar: BarData, price: float, ema_fast: float, 
                                     ema_slow: float, rsi: float, macd: float, macd_signal: float,
                                     bb_upper: float, bb_lower: float, atr: float,
                                     breakout_analysis: Dict, reversal_analysis: Dict,
                                     first_hour_levels: Optional[FirstHourLevels]) -> Tuple[str, float, str, str]:
    ...
    def create_detailed_trade_chart(self, save_path: str = "detailed_trade_analysis.png"):
        ...
        for i, bar in enumerate(self.bars):
            ...
            if first_hour_levels:
                ...
                ax1.axhline(y=first_hour_levels.first_hour_high, 
                           color=self.colors['first_hour_high'], alpha=0.7, 
                           linestyle='--', linewidth=1, label='First Hour High' if i == 0 else "")
                ax1.axhline(y=first_hour_levels.first_hour_low, 
                           color=self.colors['first_hour_low'], alpha=0.7, 
                           linestyle='--', linewidth=1, label='First Hour Low' if i == 0 else "")
        ...
        if signal_indices:
            ...
            ax1.plot(signal_indices, ema_fast_values, 
                    label='EMA Fast', color=self.colors['ema_fast'], linewidth=2)
            ax1.plot(signal_indices, ema_slow_values, 
                    label='EMA Slow', color=self.colors['ema_slow'], linewidth=2)
            ax1.plot(signal_indices, bb_upper_values, 
                    label='BB Upper', color=self.colors['bb_upper'], linewidth=1, alpha=0.7)
            ax1.plot(signal_indices, bb_lower_values, 
                    label='BB Lower', color=self.colors['bb_lower'], linewidth=1, alpha=0.7)
            ax1.plot(signal_indices, bb_middle_values, 
                    label='BB Middle', color=self.colors['bb_middle'], linewidth=1, alpha=0.5)
        ...
        if buy_signals:
            buy_indices = [timestamps.index(s.timestamp) for s in buy_signals if s.timestamp in timestamps]
            buy_prices = [s.price for s in buy_signals if s.timestamp in timestamps]
            buy_types = [s.signal_type for s in buy_signals if s.timestamp in timestamps]
            
            # Color code by signal type
            for i, (idx, price, signal_type) in enumerate(zip(buy_indices, buy_prices, buy_types)):
                ...
                ax1.scatter(idx, price, marker=marker, color=color, s=200, 
                           zorder=5, edgecolors='white', linewidth=1)
        
        if sell_signals:
            sell_indices = [timestamps.index(s.timestamp) for s in sell_signals if s.timestamp in timestamps]
            sell_prices = [s.price for s in sell_signals if s.timestamp in timestamps]
            sell_types = [s.signal_type for s in sell_signals if s.timestamp in timestamps]
            
            for i, (idx, price, signal_type) in enumerate(zip(sell_indices, sell_prices, sell_types)):
                if signal_type == 'BREAKOUT':
                    color = self.colors['breakout']
                    marker = 'v'
                elif signal_type == 'REVERSAL':
        ...
        # Format main chart
        ax1.set_title(f'{self.symbol} - Detailed Trade Analysis', 
        ...
        if signal_indices:
            ...
            ax2.plot(signal_indices, rsi_values, label='RSI', color=self.colors['rsi'], linewidth=2)
            ax2.axhline(y=70, color=self.colors['candle_red'], linestyle='--', alpha=0.5, label='Overbought')
            ax2.axhline(y=30, color=self.colors['candle_green'], linestyle='--', alpha=0.5, label='Oversold')
        ...
        if signal_indices:
            ...
            ax3.plot(signal_indices, macd_values, label='MACD', color=self.colors['macd'], linewidth=2)
            ax3.plot(signal_indices, macd_signal_values, label='Signal', color=self.colors['macd_signal'], linewidth=2)
            ax3.bar(signal_indices, 
                   [m - s for m, s in zip(macd_values, macd_signal_values)], 
                   alpha=0.3, label='Histogram', color=self.colors['volume'])
    ...
    def _print_detailed_trade_insights(self):
        ...
        print(f"   Breakout Signals: {len(breakout_signals)} ({len(breakout_signals)/total_signals*100:.1f}%)")
        print(f"   Reversal Signals: {len(reversal_signals)} ({len(reversal_signals)/total_signals*100:.1f}%)")
        print(f"   Trend Following: {len(trend_signals)} ({len(trend_signals)/total_signals*100:.1f}%)")
        ...
        print(f"\n📈 PATTERN ANALYSIS:")
        print(f"   Breakout Confirmed: {len(breakout_confirmed)} ({len(breakout_confirmed)/total_signals*100:.1f}%)")
        print(f"   15-min Reversals: {len(reversal_detected)} ({len(reversal_detected)/total_signals*100:.1f}%)")
        ...
        if buy_signals or sell_signals:
            ...
            avg_risk_reward = np.mean([s.risk_reward_ratio for s in buy_signals + sell_signals])
        ...
        if buy_signals or sell_signals:
            ...
            for i, signal in enumerate(buy_signals[:3] + sell_signals[:3]):
                print(f"   Trade {i+1}: {signal.signal} at {signal.timestamp.strftime('%H:%M')}")
