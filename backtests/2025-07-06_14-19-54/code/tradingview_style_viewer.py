...
class TradingViewStyleViewer:
    ...
    def _process_bar(self, bar: BarData):
        ...
        signal, confidence = self._generate_signal(
            ...
            bb_upper[current_idx], bb_lower[current_idx]
        )
    ...
    def _generate_signal(self, price: float, ema_fast: float, ema_slow: float, 
                        ...
                        bb_upper: float, bb_lower: float) -> Tuple[str, float]:
    ...
    def create_weekly_slides(self, save_path: str = "tradingview_weekly_slides.png"):
        ...
        fig.suptitle(f'{self.symbol} - Weekly Strategy Analysis', 
    ...
    def _create_weekly_chart(self, ax, week_data: List[Tuple[BarData, SignalData]], 
                            week_start: datetime, week_num: int):
        ...
        if signals:
            ...
            ax.plot(range(len(signals)), ema_fast_values, 
                   label='EMA Fast', color=self.colors['ema_fast'], linewidth=2)
            ax.plot(range(len(signals)), ema_slow_values, 
        ...
        if buy_signals:
            ...
            ax.scatter(buy_indices, buy_prices, 
                      marker='^', color=self.colors['candle_green'], s=200, 
                      label='Buy Signal', zorder=5, edgecolors='white', linewidth=1)
        
        if sell_signals:
            sell_indices = [signals.index(s) for s in sell_signals]
            sell_prices = [s.price for s in sell_signals]
            ax.scatter(sell_indices, sell_prices, 
                      marker='v', color=self.colors['candle_red'], s=200, 
                      label='Sell Signal', zorder=5, edgecolors='white', linewidth=1)
        
        # Format chart
        ax.set_title(f'Week {week_num} - {week_start.strftime("%B %d, %Y")}', 
    ...
    def create_comprehensive_chart(self, save_path: str = "tradingview_comprehensive.png"):
        ...
        # Format main chart
        ax1.set_title(f'{self.symbol} - Comprehensive Strategy Analysis', 
    ...
    def _print_comprehensive_statistics(self):
        ...
        if buy_signals > 0 or sell_signals > 0:
            print(f"\n🎯 PERFORMANCE METRICS:")
            print(f"   Signal Frequency: {(buy_signals + sell_signals) / total_signals * 100:.1f}%")
            print(f"   Buy/Sell Ratio: {buy_signals / max(sell_signals, 1):.2f}")
            
            # Calculate potential profit (simplified)
            if buy_signals > 0:
                avg_buy_price = np.mean([s.price for s in self.signals if s.signal == 'BUY'])
