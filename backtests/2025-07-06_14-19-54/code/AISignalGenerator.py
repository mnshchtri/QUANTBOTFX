...
class AISignalGenerator:
    ...
    def extract_features(self, current_price, current_volume):
        ...
        if len(self.volume_history) > 0:
            volume_ratio = current_volume / np.mean(self.volume_history[-20:]) if len(self.volume_history) >= 20 else 1
    ...
    def train_model(self, historical_data):
        ...
        for i in range(20, len(historical_data) - 1):
            ...
            features = self.extract_features(historical_data[i]['price'], historical_data[i].get('volume', 0))
            
            # Create label based on future price movement
            future_return = (historical_data[i + 1]['price'] - historical_data[i]['price']) / historical_data[i]['price']
    ...
    def save_model(self, filepath):
        ...
        if self.is_trained:
            model_data = {
                'model': self.model,
                'scaler': self.scaler,
                'feature_names': ['returns', 'volatility', 'ema_ratio', 'ema_trend', 
                                'rsi', 'macd', 'macd_signal', 'volume_ratio', 
                                'market_hours', 'high_activity_hours']
            }
