...
class MultiTimeframeStrategy:
    ...
    def _open_position(self, signal: SignalData, bar: BarData):
        ...
        print(f"🟢 OPEN {signal.signal} at {entry_date} {entry_time} EST @ {bar.close:.5f}")
    ...
    def _process_15min_bar(self, bar: BarData):
        ...
        signal, confidence, trade_reason = self._generate_multi_timeframe_signal(
            hour_breakout, hour_stoch_k, hour_stoch_d,
            min15_reversal, min15_stoch_k, min15_stoch_d
        )
        ...
        signal_data = SignalData(
            ...
            min15_stoch_crossover=self._get_stoch_crossover(min15_stoch_k, min15_stoch_d),
    ...
    def _analyze_1hour_conditions(self, current_bar: BarData) -> Tuple[str, float, float]:
        ...
        hour_prices = [b.close for b in self.bars_1hour[-50:]]  # Use last 50 hours for better calculation
    ...
    def _calculate_stochastic_rsi(self, prices: List[float]) -> Tuple[float, float]:
        ...
        for i in range(stoch_period - 1, len(rsi_values)):
            ...
            if highest_high == lowest_low:
                ...
                k_val = ((recent_rsi[-1] - lowest_low) / (highest_high - lowest_low)) * 100
    ...
    def _calculate_rsi_tradingview(self, prices: List[float], period: int = 14) -> List[float]:
    ...
    def _generate_multi_timeframe_signal(self, hour_breakout: str, hour_stoch_k: float, hour_stoch_d: float,
                                       min15_reversal: str, min15_stoch_k: float, min15_stoch_d: float) -> Tuple[str, float, str]:
        """Generate signal based on multi-timeframe logic"""
        ...
        if hour_breakout == "BULLISH" and hour_stoch_condition == "BULLISH":
            ...
            if min15_reversal == "BULLISH":
                return "BUY", 0.9, "Bullish breakout + bullish Stoch RSI + 15min bullish reversal"
            else:
                return "HOLD", 0.7, "Bullish breakout + bullish Stoch RSI (waiting for 15min reversal)"
            ...
            if min15_reversal == "BEARISH":
                return "SELL", 0.9, "Bearish breakout + bearish Stoch RSI + 15min bearish reversal"
            else:
                return "HOLD", 0.7, "Bearish breakout + bearish Stoch RSI (waiting for 15min reversal)"
    ...
    def create_combined_chart(self, save_path: str = "combined_timeframe_chart.png"):
        ...
        bars_15min = self.bars_15min[-96:] if len(self.bars_15min) > 96 else self.bars_15min
        ...
        for i, (timestamp, open_price, high, low, close) in enumerate(zip(timestamps_1h, opens_1h, highs_1h, lows_1h, closes_1h)):
            color = self.colors['candle_green'] if close >= open_price else self.colors['candle_red']
            
            from matplotlib.patches import Rectangle
            ax1_1hour.add_patch(Rectangle(
                (i - 0.3, min(open_price, close)), 0.6, abs(close - open_price),
        ...
        for i, bar in enumerate(bars_1hour):
            ...
            if bar_date != current_date:
                ...
                if len(day_15min_bars) >= 8:
                    ...
                    ax1_1hour.axhline(y=first_two_hours_high, color='#ff9800', 
                                     linestyle='--', linewidth=2, alpha=0.7, 
                                     label=f'First 2H High ({bar_date})' if i == 0 else "")
                    ax1_1hour.axhline(y=first_two_hours_low, color='#ff9800', 
                                     linestyle=':', linewidth=2, alpha=0.7,
        ...
        if buy_signals_1h:
            buy_indices = [timestamps_1h.index(s.timestamp) for s in buy_signals_1h if s.timestamp in timestamps_1h]
            buy_prices = [s.price for s in buy_signals_1h if s.timestamp in timestamps_1h]
            if buy_indices:
                ax1_1hour.scatter(buy_indices, buy_prices, 
                               marker='^', color=self.colors['buy_signal'], s=150, 
                               label='Buy Signal', zorder=5, edgecolors='white', linewidth=2)
        
        if sell_signals_1h:
            sell_indices = [timestamps_1h.index(s.timestamp) for s in sell_signals_1h if s.timestamp in timestamps_1h]
            sell_prices = [s.price for s in sell_signals_1h if s.timestamp in timestamps_1h]
            if sell_indices:
                ax1_1hour.scatter(sell_indices, sell_prices, 
                               marker='v', color=self.colors['sell_signal'], s=150, 
                               label='Sell Signal', zorder=5, edgecolors='white', linewidth=2)
        
        # Format 1-hour price chart
        ax1_1hour.set_title('1-Hour Breakout Analysis', color=self.colors['text'], fontsize=14, fontweight='bold')
        ...
        ax2_1hour.plot(range(len(bars_1hour)), hour_stoch_k_values, label='1H Stoch K', color='#2196f3', linewidth=1.5)
        ax2_1hour.plot(range(len(bars_1hour)), hour_stoch_d_values, label='1H Stoch D', color='#f44336', linewidth=1.5)
        ...
        ax2_1hour.axhline(y=50, color='#9e9e9e', linestyle=':', alpha=0.5, linewidth=0.5)
        ...
        for i, (timestamp, open_price, high, low, close) in enumerate(zip(timestamps_15m, opens_15m, highs_15m, lows_15m, closes_15m)):
            color = self.colors['candle_green'] if close >= open_price else self.colors['candle_red']
            
            from matplotlib.patches import Rectangle
            ax1_15min.add_patch(Rectangle(
                (i - 0.3, min(open_price, close)), 0.6, abs(close - open_price),
                facecolor=color, alpha=0.8, edgecolor=color, linewidth=1
            ))
        ...
        if buy_signals_15m:
            buy_indices = [timestamps_15m.index(s.timestamp) for s in buy_signals_15m if s.timestamp in timestamps_15m]
            buy_prices = [s.price for s in buy_signals_15m if s.timestamp in timestamps_15m]
            if buy_indices:
                ax1_15min.scatter(buy_indices, buy_prices, 
                               marker='^', color=self.colors['buy_signal'], s=150, 
                               label='Buy Signal', zorder=5, edgecolors='white', linewidth=2)
        
        if sell_signals_15m:
            sell_indices = [timestamps_15m.index(s.timestamp) for s in sell_signals_15m if s.timestamp in timestamps_15m]
            sell_prices = [s.price for s in sell_signals_15m if s.timestamp in timestamps_15m]
            if sell_indices:
                ax1_15min.scatter(sell_indices, sell_prices, 
                               marker='v', color=self.colors['sell_signal'], s=150, 
                               label='Sell Signal', zorder=5, edgecolors='white', linewidth=2)
        
        # Format 15-minute price chart
        ax1_15min.set_title('15-Minute Reversal Analysis', color=self.colors['text'], fontsize=14, fontweight='bold')
        ...
        ax2_15min.plot(range(len(bars_15min)), min15_stoch_k_values, label='15M Stoch K', color='#2196f3', linewidth=1.5)
        ax2_15min.plot(range(len(bars_15min)), min15_stoch_d_values, label='15M Stoch D', color='#f44336', linewidth=1.5)
        ...
        ax2_15min.axhline(y=50, color='#9e9e9e', linestyle=':', alpha=0.5, linewidth=0.5)
        ...
        if len(timestamps_1h) > 0:
            ...
            tick_labels_1h = [timestamps_1h[i].strftime('%H:%M') for i in tick_positions_1h]
            ax2_1hour.set_xticks(tick_positions_1h)
            ax2_1hour.set_xticklabels(tick_labels_1h, rotation=45, color=self.colors['text'])
        
        if len(timestamps_15m) > 0:
            step_15m = max(1, len(timestamps_15m) // 8)
            tick_positions_15m = range(0, len(timestamps_15m), step_15m)
            tick_labels_15m = [timestamps_15m[i].strftime('%H:%M') for i in tick_positions_15m]
            ax2_15min.set_xticks(tick_positions_15m)
            ax2_15min.set_xticklabels(tick_labels_15m, rotation=45, color=self.colors['text'])
        ...
        # Add main title
        fig.suptitle(f'{self.symbol} - Multi-Timeframe Analysis', 
                    color=self.colors['text'], fontsize=18, fontweight='bold', y=0.98)
        
        plt.tight_layout()
        plt.savefig(save_path, dpi=300, bbox_inches='tight', facecolor=self.colors['background'])
        plt.show()

def generate_multi_timeframe_data(symbol: str = "EURUSD", days: int = 7) -> Tuple[List[BarData], List[BarData]]:
    ...
    for bar_15min in range(total_15min_bars):
        # Create some realistic price movement
        timestamp = datetime.now() - timedelta(minutes=15*total_15min_bars-bar_15min*15)
    ...
    print(f"Generated {len(bars_1hour)} 1-hour bars and {len(bars_15min)} 15-minute bars")
