...
class DataConfig:
    ...
    def is_cached_data_available(
        self, instrument: str, timeframe: str, days_back: int
    ) -> bool:
    ...
    def load_cached_data(
        ...
        try:
            ...
            logger.info(
                f"📁 Loaded cached data: {len(df)} candles for {instrument} {timeframe}"
            )
            return df
    ...
    def save_data_to_cache(
        self, df: pd.DataFrame, instrument: str, timeframe: str, days_back: int
    ):
        """Save data to cache"""
        cache_path = self.get_cache_path(instrument, timeframe, days_back)
        ...
        try:
            ...
            logger.info(
                f"💾 Cached data: {len(df)} candles for {instrument} {timeframe}"
            )
        except Exception as e:
            logger.error(f"⚠️ Error caching data: {e}")

    def get_data_loading_strategy(
        self, instrument: str, timeframe: str, days_back: int
    ) -> str:
...
def set_data_source(source: str) -> Dict[str, Any]:
    ...
    if source.lower() not in valid_sources:
        return {
            "status": "error",
            "message": f"Invalid data source. Must be one of: {valid_sources}",
        }
