...
async def register_strategy(request: StrategyConfigRequest):
    ...
    try:
        ...
        result = strategy_manager.register_strategy(
            ...
            {"parameters": request.parameters, "description": request.description},
        )
...
async def update_strategy_parameters(
    strategy_name: str, request: StrategyParameterUpdateRequest
):
    """Update strategy parameters"""
    try:
        strategy_manager = get_strategy_manager()
        result = strategy_manager.update_strategy_parameters(
            strategy_name, request.parameters
        )
...
async def execute_strategy(request: StrategyExecutionRequest):
    ...
    try:
        ...
        if not data_df:
            raise HTTPException(
                status_code=400, detail="No data available for strategy execution"
            )

        # Execute strategy
        results = strategy_manager.execute_strategies_on_data(
            data_df, request.data_index
        )
                ...
                "execution_results": [
                    {
                        ...
                        "signal": {
                            "type": r.signal.signal_type.value
                            if r.signal and hasattr(r.signal.signal_type, "value")
                            else str(r.signal.signal_type)
                            if r.signal
                            else None,
                            "price": getattr(
                                r.signal, "price", getattr(r.signal, "entry_price", 0)
                            )
                            if r.signal
                            else None,
...
async def get_strategy_signals(strategy_name: str):
    ...
    try:
        ...
        if not data_df:
            raise HTTPException(
                status_code=400, detail="No data available for signal generation"
            )
...
async def clear_execution_history():
    ...
    try:
        ...
        return {"success": True, "message": "Execution history cleared", "data": {}}
