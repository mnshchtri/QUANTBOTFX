...
async def execute_trade(order: Order):
    ...
    return {"success": True, "message": f"Order executed for {order.symbol}", "position_id": positions[-1]["id"]}

@router.get("/summary")
async def get_account_summary():
    # Mock account summary
    total_balance = 100000.0
    equity = total_balance + sum([p["volume"] * 10 for p in positions]) # Simple mock PnL
    return {
        "success": True,
        "data": {
            ...
            "pnl_percent": (sum([p["volume"] * 10 for p in positions]) / total_balance) * 100
        }
    }
