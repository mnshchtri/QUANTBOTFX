...
def _load_oanda_settings():
    ...
    try:
        ...
        base_url = oanda_cfg.get("base_url") or os.getenv("OANDA_BASE_URL") or "https://api-fxpractice.oanda.com"
...
def fetch_oanda_data(instrument: str, granularity: str, count: int = 500) -> pd.DataFrame:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if not candles:
                raise HTTPException(status_code=404, detail=f"No data available for {instrument}")
            ...
            if not rows:
                raise HTTPException(status_code=404, detail=f"No complete candles available for {instrument}")
...
def get_candles_data(instrument: str, timeframe: str, limit: int = Query(1000, ge=1, le=5000)):
    ...
    try:
        ...
        logger.info(f"✅ Successfully returned {len(data)} candles for {instrument} {timeframe}")
