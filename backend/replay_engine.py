...
class ReplayEngine:
    ...
    def load_data(
        self, instrument: str, start_date: str, end_date: str, timeframe: str = "M15"
    ):
        """Load historical data for replay - DEPRECATED: Use load_backtest_data instead"""
        logger.warning(
            "load_data() is deprecated. Use load_backtest_data() with proper data structure instead."
        )
        return False
    ...
    def load_backtest_data(self, replay_export: Dict):
        ...
        try:
            ...
            logger.info(
                f"Loading data type: {type(data)}, length: {len(data) if hasattr(data, '__len__') else 'unknown'}"
            )
            ...
            logger.info(
                f"Final data type: {type(self.data)}, shape: {getattr(self.data, 'shape', 'N/A')}"
            )
            ...
            if "trades" in replay_export and "signals" in replay_export:
                ...
                self.backtest_metadata = {
                    "strategy_name": getattr(
                        self.strategy, "name", "Range of the Day Strategy"
                    ),
            ...
            # Setup replay state
            self.current_index = (
                len(self.data) - 1 if len(self.data) > 0 else 0
            ...
            logger.info(
                f"   📊 {len(self.data)} candles ({self.backtest_metadata['timeframe']})"
            )
            logger.info(
                f"   🎯 {len(self.signals)} signals from {self.backtest_metadata['strategy_name']}"
            )
            ...
            if not self.data.empty:
                ...
                logger.info(
                    f"   📈 Data range: {self.data.index[0]} to {self.data.index[-1]}"
                )
    ...
    def get_current_replay_state(self):
        ...
        for trade in self.trades:
            if (
                trade["entry_index"] <= self.current_index
                and trade["exit_index"] > self.current_index
            ):
    ...
    def _update_performance(self):
        ...
        for trade in self.trades:
            if trade["status"] == "closed":
                total_pnl += (
                    trade.get("profit_loss_pips", 0) * trade["position_size"] / 10000
                )
    ...
    def get_current_data(self) -> Tuple[pd.DataFrame, List, List]:
        ...
        logger.info(
            f"get_current_data called - is_playing: {self.is_playing}, current_index: {self.current_index}"
        )
    ...
    def _create_display_dataframe(self) -> pd.DataFrame:
        ...
        logger.info(
            f"Creating display DataFrame - is_playing: {self.is_playing}, current_index: {self.current_index}"
        )
        ...
        if self.current_index < len(self.data) - 1:
            ...
            logger.info(
                f"Replay position {self.current_index}: showing {len(display_data)} candles (playing: {self.is_playing})"
            )
            return display_data
        else:
    ...
    def get_performance_metrics(self) -> Dict:
        ...
        if len(self.performance_history) > 1:
            returns = [
                p["balance"] - self.performance_history[i - 1]["balance"]
    ...
    def jump_to_date(self, target_date: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Closest index: {closest_index}, time difference: {time_diff[closest_index]}"
            )

            if 0 <= closest_index < len(self.data):
                self.current_index = closest_index
                logger.info(
                    f"Jumped to index {closest_index}, date: {data_index[closest_index]}"
                )
                return True
            else:
                logger.error(
                    f"Closest index {closest_index} is out of range [0, {len(self.data)})"
                )
                return False
        except Exception as e:
    ...
    def set_start_position_with_context(
        self, target_date: str, context_candles: int = 14
    ) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Target index: {target_index}, date: {data_index[target_index]}"
            )
            ...
            if 0 <= target_index < len(self.data):
                ...
                logger.info(
                    f"Set start position: context from index {start_index}, play will start at index {target_index}"
                )
                return True
            else:
                logger.error(
                    f"Target index {target_index} is out of range [0, {len(self.data)})"
                )
                return False
        except Exception as e:
    ...
    def get_replay_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_date": str(self.data.index[self.current_index])
