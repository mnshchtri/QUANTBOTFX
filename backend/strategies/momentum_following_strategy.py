...
class RangeOfTheDayStrategy(BaseStrategy):
    ...
    def __init__(self):
        super().__init__("Range of the Day Strategy", "Momentum following strategy with stochastic analysis")
    ...
    def _generate_signals(self, data: pd.DataFrame, stoch_k: pd.Series, 
                         stoch_d: pd.Series, rsi: pd.Series) -> List[TradeSignal]:
        """
    ...
    def process_historical_data(self, df: pd.DataFrame, instrument: str, granularity: str = 'M15') -> pd.DataFrame:
        """
    ...
    def validate_historical_data(self, df, instrument, granularity):
        ...
        if len(gaps) > 0:
            print(f"⚠️  Found {len(gaps)} remaining data gaps in {instrument} {granularity}")
            print(f"   Largest gap: {gaps.max()}")
        
        print(f"✅ {instrument} {granularity}: {len(df)} candles from {df.index[0]} to {df.index[-1]}")
        return True
    ...
    def _remove_non_trading_gaps(self, df, instrument, granularity):
        ...
        holiday_dates = [datetime.strptime(date, '%Y-%m-%d').date() for date in holidays_2025]
        ...
        if removed_count > 0:
            print(f"🧹 Removed {removed_count} non-trading candles from {instrument} {granularity}")
    ...
    def get_candles_df(self, instrument, count=5000, granularity='M15', from_time=None, to_time=None):
