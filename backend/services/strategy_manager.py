...
class StrategyManager:
    ...
    def register_strategy(self, strategy_name: str, strategy_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            strategy_instance = create_strategy_instance(strategy_name, **strategy_config.get("parameters", {}))
    ...
    def execute_strategies_on_data(self, data: pd.DataFrame, current_index: int) -> List[StrategyExecutionResult]:
        ...
        current_data = data.iloc[:current_index + 1] if current_index < len(data) else data
        ...
        for strategy_name, strategy_info in self.active_strategies.items():
            try:
                ...
                current_point = data.iloc[current_index] if current_index < len(data) else {}
                
                # Create execution result
                execution_result = StrategyExecutionResult(
                    timestamp=datetime.now(),
                    signal=result.signals[-1] if result.signals else None,
                    data_point=current_point.to_dict() if hasattr(current_point, 'to_dict') else {},
                    strategy_name=strategy_name,
                ...
                logger.debug(f"Strategy {strategy_name} executed in {execution_time:.2f}ms")
    ...
    def get_strategy_signals(self, strategy_name: str, data: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for i, signal in enumerate(result.signals):
                signals.append({
                    ...
                    'type': signal.signal_type.value.lower() if hasattr(signal.signal_type, 'value') else str(signal.signal_type),
                    'price': getattr(signal, 'price', getattr(signal, 'entry_price', 0)),
    ...
    def get_all_strategy_signals(self, data: pd.DataFrame) -> Dict[str, List[Dict[str, Any]]]:
    ...
    def update_strategy_parameters(self, strategy_name: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
