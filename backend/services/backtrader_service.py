...
class BacktraderService:
            ...
            for result_file in self.backtest_results_dir.glob("*.json"):
                try:
                    with open(result_file, "r") as f:
                        data = json.load(f)
                        logger.info(
                            f"📊 Loaded backtest result: {data.get('strategy_name', 'Unknown')}"
                        )
                ...
                try:
                    with open(result_file, "r") as f:
                        ...
                        if (
                            ...
                            or data.get("strategy_name") == strategy_name
                        ):
                            results.append(data)
    ...
    def get_strategy_signals(
        ...
        try:
            ...
            if not results:
                logger.warning(
                    f"No backtest results found for strategy: {strategy_name}"
                )
                return []
            ...
            if not matching_results:
                logger.warning(
                    f"No backtest results found for {strategy_name} on {instrument} {timeframe}"
                )
                return []

            # Use the most recent result
            latest_result = max(
                matching_results, key=lambda x: x.get("backtest_date", "")
            )
            ...
            logger.info(
                f"📊 Found {len(filtered_signals)} signals for {strategy_name} in date range"
            )
            ...
            for result_file in self.backtest_results_dir.glob("*.json"):
                try:
                    with open(result_file, "r") as f:
                        ...
                        if (
                            data.get("instrument") == instrument
    ...
    def create_aligned_backtest(
        ...
        try:
            ...
            if result["success"]:
                logger.info(
                    f"✅ Aligned backtest created successfully for {strategy_name}"
                )
            else:
