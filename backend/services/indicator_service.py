...
class IndicatorService:
    ...
    async def get_data_from_management_ai(self, symbol: str, timeframe: str, limit: int = 1000) -> pd.DataFrame:
    ...
    def _get_cache_key(self, symbol: str, timeframe: str, indicators: List[str], parameters: Dict[str, Any]) -> str:
    ...
    async def calculate_indicators(
        ...
        try:
            ...
            for indicator in indicators:
                try:
                    ...
                    if indicator.lower() in ['rsi']:
                        ...
                        df_with_indicator = self.indicator_lib.calculate_rsi(df, period=period)
                        ...
                        df_with_indicator = self.indicator_lib.calculate_sma(df, period=period)
                        ...
                        period = indicator_params.get('period', 20)
                        df_with_indicator = self.indicator_lib.calculate_ema(df, period=period)
            ...
            logger.info(f"📊 Calculated {len(result_indicators)} indicators for {symbol} {timeframe}")
            return result
