...
class ReplayService:
    ...
    def _start_stepping_thread(self):
        ...
        self.stepping_thread = threading.Thread(
            target=self._stepping_worker, daemon=True
        )
    ...
    async def get_data_from_backend_api(
        ...
        try:
            ...
            logger.info(
                f"🌐 Fetching data directly from OANDA: {instrument} {timeframe}"
            )
            ...
            if response.status_code == 200:
                ...
                if not rows:
                    raise ValueError(
                        f"No complete candles available for {instrument} {timeframe}"
                    )
    ...
    async def initialize_replay(
        self, instrument: str = "GBP_JPY", timeframe: str = "M15", days_back: int = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"📊 Backend URL: {self.backend_url}/api/trading-data/{instrument}/{timeframe}"
            )
            ...
            logger.info(
                f"📊 Received data shape: {market_data.shape if hasattr(market_data, 'shape') else 'No shape'}"
            )
            if not market_data.empty:
                logger.info(f"📊 First candle: {market_data.iloc[0]}")
                logger.info(f"📊 Last candle: {market_data.iloc[-1]}")
                logger.info(
                    f"📊 Data range: {market_data.index[0]} to {market_data.index[-1]}"
                )
            ...
            if market_data.empty:
                raise ValueError(
                    f"No market data available for {instrument} {timeframe}"
                )
            ...
            logger.info(
                f"📊 Created replay export with {len(market_data)} candles for {instrument} {timeframe}"
            )
            ...
            if success:
                logger.info(
                    f"✅ Replay initialized successfully for {instrument} {timeframe}"
                )
                return {
                    "success": True,
                    "message": f"Replay initialized successfully for {instrument} {timeframe}",
                    "total_candles": len(self.replay_engine.data)
                    if self.replay_engine.data is not None
                    and not self.replay_engine.data.empty
                    else 0,
                    "signals": len(self.replay_engine.signals)
                    if hasattr(self.replay_engine, "signals")
                    and self.replay_engine.signals is not None
                    else 0,
                }
            else:
    ...
    def control_replay(self, action: str, speed: float = 1.0) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "data": {"action": action, "speed": self.replay_speed},
            }
    ...
    def get_data_range(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "data": data_range}
    ...
    def add_strategy_overlay(self, strategy_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if signals:
                ...
                logger.info(
                    f"🎯 Added {len(signals)} strategy signals as overlay for {strategy_name}"
                )
    ...
    def _load_strategy_backtest_signals(
        ...
        try:
            ...
            logger.info(
                f"🔍 Loading backtest signals for {strategy_name} on {instrument} {timeframe}"
            )
    ...
    def set_start_position(
        ...
        context_candles: int = 50,
    ) -> Dict[str, Any]:
        """Set replay start position by date or position index"""
        try:
            logger.info(
                f"🎯 Setting replay start position: date={date}, position={position}"
            )

            if self.replay_engine.data is None or self.replay_engine.data.empty:
                return {"success": False, "message": "No replay data available"}
            ...
            if date:
                ...
                try:
                    ...
                    if (
                        hasattr(self.replay_engine.data.index, "tz")
                        and self.replay_engine.data.index.tz is not None
                    ):
                    ...
                    return {"success": False, "message": f"Invalid date format: {date}"}
            ...
            logger.info(
                f"✅ Replay position set to index {target_index} (date: {target_date})"
            )
